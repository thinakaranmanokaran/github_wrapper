...
const HomeTop = ({ query, setQuery, handleClear, results, loading, handleSelectUser, error }) => {
    ...
    const pinnedUser = "thinakaranmanokaran"

    //Fetch the user for pinnedUser
    async function fetchPinnedUser() {
        try {
            ...
            <div className="text-center relative">
                ...
                <div className="flex flex-col items-center my-6 gap-2 w-full group">
                    <div className="relative w-fit ">
                        ...
                        {/* Dropdown */}
                        {(pinUser || results.length > 0) && (
                            <ul className={`absolute left-0 right-0 mt-2 dark:bg-black bg-white  border-neutral-600 shadow-lg max-h-80 overflow-y-auto z-20 ${(focused || query) && (results.length > 0 || pinUser) && "border"} ${pinUser ? "py-0" : "py-2"}`}>

                                {/* 🔖 PINNED USER (only when input is empty) */}
                                {focused && pinUser && query.length === 0 && (
                                    <li
                                        onClick={() => handleSelectUser(pinUser.login)}
                                        className="group/item flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-light dark:hover:bg-neutral-800 transition"
                                    >

                                        <img
                                            src={pinUser.avatar_url}
                                            alt={pinUser.login}
                                ...
                                {results.slice(0, 5).map((user) => (
                                    <li
                                        ...
                                        className="group/item flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-light dark:hover:bg-neutral-800 transition"
                                    >
                                        <img
                                            src={user.avatar_url}
                                            alt={user.login}
                                        ...
                                        <IoArrowForward
                                            className="ml-auto text-neutral-600 transition-transform group-hover/item:translate-x-2"
                                        />
                                    </li>
                                ))}
                            </ul>
